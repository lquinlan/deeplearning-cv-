...
class Filter_w():
    ...
    def ideal_low_filter(self):
        ...
        fshift=self.my_fft()
        d=self.H**2
        flag=np.array([((i-self.center[0])**2+(j-self.center[1])**2)<d for i in range(0,self.shape[0]) \
                       for j in range(0,self.shape[1])]).reshape(self.shape)
        self.mask[flag]=1
...
for i in range(0,img.shape[2]):
    h_filter1=Filter_w(img[:,:,i],8)
    new_img[:,:,i]=h_filter1.ideal_high_filter()
for i in range(0,img.shape[2]):
    l_filter1=Filter_w(img1[:,:,i],8)
    new_img1[:,:,i]=l_filter1.ideal_low_filter()
plt.subplot(121)
plt.imshow(new_img)
plt.title('cat')
plt.subplot(122)
...
new_img2=new_img1+new_img
new_img2=new_img2/255
dst1=cv.pyrDown(new_img2)
dst2=cv.pyrDown(dst1)
dst3=cv.pyrDown(dst2)
size=list(new_img2.shape)
size[1]+=dst1.shape[1]+dst2.shape[1]+dst3.shape[1]
res=np.zeros(size)
res[0:new_img2.shape[0],0:new_img2.shape[1],:]=new_img2
res[new_img2.shape[0]-dst1.shape[0]:new_img2.shape[0],new_img2.shape[1]:new_img2.shape[1]+dst1.shape[1],:]=dst1
res[new_img2.shape[0]-dst2.shape[0]:new_img2.shape[0],new_img2.shape[1]+dst1.shape[1]:new_img2.shape[1]+dst1.shape[1]+dst2.shape[1],:]=dst2
res[new_img2.shape[0]-dst3.shape[0]:new_img2.shape[0],new_img2.shape[1]+dst1.shape[1]+dst2.shape[1]:new_img2.shape[1]+dst1.shape[1]+dst2.shape[1]+dst3.shape[1],:]=dst3

# plt.subplot(224)
plt.imshow(res)
plt.show()

# max_filter=Filter_w(img,'max',(3,3),axis=0)
# new_img=max_filter.laplace()
# from matplotlib.gridspec import GridSpec 
# fig = plt.figure()

# gs = GridSpec(800, 800)   # 把Figure分成40*40的网格图
# # ax1是axes对象，这一步意思是ax1画的图在原图(40*40)占据行5-14，占据列5-14(从零开始索引)
# ax1 = fig.add_subplot(gs[0:620, 0:560])
# ax2 = fig.add_subplot(gs[300:620, 560:800])   
